...
import cube.ware.service.message.recent.adapter.RecentSessionAdapter;
        ...
        mRecentSessionAdapter.setOnItemClickListener(new BaseQuickAdapter.OnItemClickListener() {
            ...
            public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
                ...
                if (recentSession.sessionType == CubeSessionType.Group.getType()) {
                    ARouter.getInstance().build(CubeConstants.Router.GroupChatActivity).withString(EXTRA_CHAT_ID, recentSession.sessionId).withString(EXTRA_CHAT_NAME, TextUtils.isEmpty(recentSession.displayName) ? recentSession.sessionId : recentSession.displayName).withSerializable(EXTRA_CHAT_CUSTOMIZATION, new GroupChatCustomization()).navigation();
                }
                else {
                    ARouter.getInstance().build(CubeConstants.Router.P2PChatActivity).withString(EXTRA_CHAT_ID, recentSession.sessionId).withString(EXTRA_CHAT_NAME, TextUtils.isEmpty(recentSession.displayName) ? recentSession.sessionId : recentSession.displayName).withSerializable(EXTRA_CHAT_CUSTOMIZATION, new P2PChatCustomization()).navigation();
                }
            }
        });
    }
    ...
    private void showMorePopWindow() {
        ...
        popupWindow.setOnDismissListener(new PopupWindow.OnDismissListener() {
            ...
            public void onDismiss() {
                ScreenUtil.setBackgroundAlpha(RecentSessionFragment.this.getActivity(), 1.0f);
            }
        });
        ...
        addFriendTv.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                popupWindow.dismiss();
                RouterUtil.navigation(RecentSessionFragment.this.getContext(), CubeConstants.Router.AddFriendActivity);
                RecentSessionFragment.this.getActivity().overridePendingTransition(R.anim.activity_open, 0);
            }
        });
        // 创建群组
