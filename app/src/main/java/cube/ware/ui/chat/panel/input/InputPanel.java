...
import cube.ware.ui.chat.panel.input.function.FunctionPanel;
import cube.ware.ui.chat.panel.input.voicefragment.RecordFragment;
...
//    private EmoticonPickerView mEmoticonPickerView;  // 贴图表情控件
    ...
    private Handler mUIHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case AppConstants.SHOW_EMOTICON_LAYOUT:
...
//                    mEmoticonPickerView.setVisibility(View.VISIBLE);
                    break;
                case AppConstants.SHOW_KEYBOARD:
                    showKeyboard();
                    break;
                case AppConstants.SHOW_MORE_FUNCTION_LAYOUT:
                    mChatMoreBtn.setSelected(true);
                    mFunctionMoreLayout.setVisibility(View.VISIBLE);
                    break;
                case AppConstants.SHOW_VOICE_LAYOUT:
                    mChatVoiceBtn.setSelected(true);
                    mVoiceLayout.setVisibility(View.VISIBLE);
                    break;
                case AppConstants.HIDE_ALL_LAYOUT:
                    hideKeyboard();
...
//        this.mEmoticonPickerView = (EmoticonPickerView) rootView.findViewById(R.id.emoticon_picker_view);
    ...
    public void onClick(View v) {
        ...
        //点击照相
            ...
            hideAllLayout(true);
            this.hasBottomNavigationListener(AppConstants.NAVIGATION_TYPE_CAMERA);
        }
        //发送文件
            ...
            RxPermissionUtil.requestStoragePermission(mChatContainer.mChatActivity).compose(RxSchedulers.<Boolean>io_main()).subscribe(new Action1<Boolean>() {
                ...
                public void call(Boolean aBoolean) {
                    if (aBoolean) {
                        hasBottomNavigationListener(AppConstants.NAVIGATION_TYPE_FILE);
                    }
                    else {
        ...
        //发送图片按钮
            ...
            hideAllLayout(true);
            this.hasBottomNavigationListener(AppConstants.NAVIGATION_TYPE_IMAGE);
        }
        //录制语音按钮
    ...
    private void sendMessage(int type) {
        ...
        if (mChatMessageEt.isInReplyMode()) {
            ...
            TextMessage textMessage = MessageManager.getInstance().buildTextMessage(mChatType, SpUtil.getCubeId(), mChatContainer.mChatId,mChatContainer.mChatName,text, isSecret);
            MessageManager.getInstance().replyMessage(mCubeMessage, textMessage);
            mChatMessageEt.getText().clear();
            mListPanel.scrollToBottom();
        }
        else {
            final TextMessage textMessage = MessageManager.getInstance().buildTextMessage(mChatType, SpUtil.getCubeId(), mChatContainer.mChatId,mChatContainer.mChatName, text, isSecret);
    ...
    private void hideFunctionMoreLayout() {
        this.mUIHandler.removeMessages(AppConstants.SHOW_MORE_FUNCTION_LAYOUT);
        if (this.mFunctionMoreLayout != null) {
    ...
    private void showFunctionMoreLayout() {
        if (mIsKeyboardShowed) {
            this.mUIHandler.sendEmptyMessageDelayed(AppConstants.SHOW_MORE_FUNCTION_LAYOUT, AppConstants.SHOW_LAYOUT_DELAY);
        }
        else {
            this.mUIHandler.sendEmptyMessage(AppConstants.SHOW_MORE_FUNCTION_LAYOUT);
        }
    ...
    private void hideVoiceLayout() {
        this.mUIHandler.removeMessages(AppConstants.SHOW_VOICE_LAYOUT);
    ...
    private void showVoiceLayout() {
        if (mIsKeyboardShowed) {
            this.mUIHandler.sendEmptyMessageDelayed(AppConstants.SHOW_VOICE_LAYOUT, AppConstants.SHOW_LAYOUT_DELAY);
        }
        else {
            this.mUIHandler.sendEmptyMessage(AppConstants.SHOW_VOICE_LAYOUT);
        }
...
//        this.mUIHandler.removeMessages(AppConstants.SHOW_EMOTICON_LAYOUT);
...
//        if (mIsKeyboardShowed) {
//            //如果当前正在显示键盘则先把键盘隐藏
//            this.mUIHandler.sendEmptyMessageDelayed(AppConstants.SHOW_EMOTICON_LAYOUT, SHOW_LAYOUT_DELAY);
//        }
//        else {
//            this.mUIHandler.sendEmptyMessage(AppConstants.SHOW_EMOTICON_LAYOUT);
//        }
//        if (type == 0){
//            this.mChatMessageEt.requestFocus();
//        }else {
...
//        this.mChatContainer.mPanelProxy.inputPanelExpanded();
    ...
    private void toggleKeyboard(boolean isShowKeyboard) {
        ...
        if (isShowKeyboard) {
            this.mUIHandler.sendEmptyMessage(AppConstants.SHOW_KEYBOARD);
        }
        else {
            this.hideKeyboard();
        }
    ...
    private void hideKeyboard() {
        ...
        this.mUIHandler.removeMessages(AppConstants.SHOW_KEYBOARD);
    ...
    public void hideAllLayout(boolean immediately) {
        ...
        this.mUIHandler.sendEmptyMessageDelayed(AppConstants.HIDE_ALL_LAYOUT, delay);
    }
...
//        }
    ...
    private void atMember(final List<String> atMemberData, final int selectionStart, final boolean hasAt) {
        ...
        //显示键盘
        this.mUIHandler.sendEmptyMessageDelayed(AppConstants.SHOW_KEYBOARD, AppConstants.SHOW_LAYOUT_DELAY);
    }
    ...
    public void onReplyMessage(CubeMessage cubeMessage) {
        ...
        Context applicationContext = CubeUI.getInstance().getContext();
    ...
    public void hasBottomNavigationListener(int navigationType) {
        if (this.mOnBottomNavigationListener != null) {
            switch (navigationType) {
                case AppConstants.NAVIGATION_TYPE_CAMERA:
                    this.mOnBottomNavigationListener.onCameraListener();
                    break;
                case AppConstants.NAVIGATION_TYPE_FILE:
                    this.mOnBottomNavigationListener.onSendFileListener();
                    break;
                case AppConstants.NAVIGATION_TYPE_IMAGE:
