...
public class FriendListAdapter extends BaseQuickAdapter<CubeUser, BaseViewHolder> {
    ...
    protected void convert(BaseViewHolder helper, CubeUser item) {
        ...
        GlideUtil.loadCircleImage(AppManager.getAvatarUrl() + item.getCubeId(), mContext, avatarView, DiskCacheStrategy.NONE, true, R.drawable.default_head_user);
        helper.setText(R.id.tv_username, TextUtils.isEmpty(item.getDisplayName()) ? item.getCubeId() : item.getDisplayName()).setText(R.id.tv_cubeId, item.getCubeId());
    }
}
