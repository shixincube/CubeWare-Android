package cube.ware.ui.recent.manager;
...
public class RecentSessionManager {
    ...
    public void addOrUpdateRecentSession(final MessageEntity messageEntity) {
        ...
        CubeRecentSessionRepository.getInstance().addOrUpdateRecentSession(cubeRecentSession)
    ...
    public void addOrUpdateRecentSession(final List<MessageEntity> messageList) {
        ...
        Observable.from(cubeRecentSessions)
                .subscribeOn(Schedulers.io())
                .flatMap(new Func1<CubeRecentSession, Observable<CubeRecentSession>>() {
                    @Override
                    public Observable<CubeRecentSession> call(CubeRecentSession cubeRecentSession) {
                        return CubeRecentSessionRepository.getInstance().queryBySessionId(cubeRecentSession.getSessionId())
                ...
                .toList()
                .flatMap(new Func1<List<CubeRecentSession>, Observable<List<CubeRecentSession>>>() {
                    @Override
    ...
    public void removeRecentSession(String sessionId){
        CubeRecentSessionRepository.getInstance().removeRecentSession(sessionId)
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Action1<CubeRecentSession>() {
                    @Override
                    public void call(CubeRecentSession cubeRecentSession) {
       ...
       /*     if (message.getSender().getCubeId().equals("10000")) {
            ...
            if (message instanceof TextMessage && message.getDirection() != MessageDirection.Sent && !message.isReceipted()) {
                ...
                if (MessageManager.getInstance().isAtMe(textMessage.getContent())) {
                    ...
                    SpUtil.setReceiveAt(AppConstants.Sp.SP_CUBE_AT + sessionId, true);
                    LogUtil.d("有人@我: " + sessionId);
                }
                else if (MessageManager.getInstance().isAtAll(textMessage.getContent()) && message.isGroupMessage()) {
                    SpUtil.setReceiveAtAll(AppConstants.Sp.SP_CUBE_RECEIVE_ATALL + sessionId, true);
    ...
    public CubeRecentSession convertTo(MessageEntity messageEntity) {
        ...
        if (SystemMessageManage.getInstance().isAddFriend(messageEntity)) {
            ...
            if (TextUtils.equals(applyCube, SpUtil.getCubeId())) {
                senderId = acceptCube;
            }
            else {
                senderId = applyCube;
        ...
        if ((senderId.equals(receiverId) && senderId.equals(SpUtil.getCubeId()))) {
            //业务层不显示自己发给自己的消息
            return null;
        }
        try {
            ...
            if (MessageManager.getInstance().isGroupMessage(messageEntity)) {
                ...
                if (TextUtils.isEmpty(SpUtil.getCubeId())) {
                ...
                if (sessionId.equals(SpUtil.getCubeId()) || SystemMessageManage.getInstance().isSystemSessionId(senderId)) {
    ...
    private String getContent(MessageEntity messageEntity) {
        ...
        String content = "";
        if (messageEntity.isGroupMessage() && !TextUtils.equals(sender.getCubeId(),SpUtil.getCubeId())) {
