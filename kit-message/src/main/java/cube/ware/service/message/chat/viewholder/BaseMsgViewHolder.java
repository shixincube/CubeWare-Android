...
public abstract class BaseMsgViewHolder implements MessagePopupManager.OnPopMenuHandleListener {
    ...
    public void update(ChatMessageAdapter adapter, BaseViewHolder viewHolder, CubeMessageViewModel data, int position, Map<String, CubeMessage> selectedMap) {
        this.mAdapter = adapter;
        this.mViewHolder = viewHolder;
        this.mData = data;
        this.mPosition = position;
    ...
    public boolean isReceivedMessage() {
        return this.mData.isReceivedMessage() && !mData.mMessage.getSenderId().equals(CubeCore.getInstance().getCubeId());
    }
    ...
    private void setStatus() {
        ...
        switch (status) {
            case Failed:
                ...
                break;
            case Sending:
                if (time > TIME_OUT && !CubeMessageType.isFileMessage(this.mData.mMessage.getMessageType())) {
                    this.mProgressBar.setVisibility(View.GONE);
                    this.mRepeatButton.setVisibility(View.VISIBLE);
    ...
    private void setChatDateByTimeStamp(int currentPos) {
        if (currentPos > 0) {
            CubeMessageViewModel cubeMessageViewModel = mAdapter.getData().get(mPosition - 1);
    ...
    public void startSecretTime(final int time) {
        ...
        //                RxBus.getInstance().post(MessageConstants.Event.EVENT_REFRESH_RECENT_SECRET_SESSION_SINGLE, mData.mMessage.getChatId());
        //                RxBus.getInstance().post(MessageConstants.Event.EVENT_REFRESH_RECENT_SESSION_SINGLE, "secret_chat");
        //            }
        //        });
    }
