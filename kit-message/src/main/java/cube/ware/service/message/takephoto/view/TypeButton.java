...
public class TypeButton extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (button_type == TYPE_CANCEL) {
            ...
            path.moveTo(center_X * 0.75f, center_Y * 0.75f);
            path.lineTo(center_X * 1.25f, center_Y * 1.25f);
            canvas.drawPath(path, mPaint);
            path.moveTo(center_X * 1.25f, center_Y * 0.75f);
            path.lineTo(center_X * 0.75f, center_Y * 1.25f);
            canvas.drawPath(path, mPaint);
        }
        //如果类型为确认，则绘制绿色勾
