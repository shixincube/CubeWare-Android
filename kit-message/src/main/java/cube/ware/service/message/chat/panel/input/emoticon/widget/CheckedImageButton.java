...
public class CheckedImageButton extends AppCompatImageButton {
    ...
    public void setChecked(boolean checked) {
        ...
        Drawable image = checked ? checkedImage : normalImage;
        if (image != null) {
            updateImage(image);
        }

        int background = checked ? checkedBkResId : normalBkResId;
        if (background != 0) {
    ...
    public void setNormalImage(File filePath) {
    ...
    public void setCheckedImage(File filePath) {
        Bitmap bitmap = BitmapFactory.decodeFile(String.valueOf(filePath));
