...
public class RecentSessionAdapter extends BaseQuickAdapter<RecentSession, BaseViewHolder> {
    ...
    protected void convert(BaseViewHolder helper, RecentSession item) {
        ...
        helper.setText(R.id.message_name_tv, item.displayName).setVisible(R.id.message_top_label_iv, item.topTime > 0).setVisible(R.id.message_badge_tv, item.unreadCount > 0).setText(R.id.message_badge_tv, item.unreadCount > 99 ? "99+" : String.valueOf(item.unreadCount)).setText(R.id.message_content_tv, item.content);
        if (item.time != -1) {
