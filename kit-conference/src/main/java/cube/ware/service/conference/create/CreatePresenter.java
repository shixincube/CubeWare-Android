...
public class CreatePresenter extends CreateContract.Presenter {
    ...
    public ConferenceConfig initConferenceConfig(String theme, String groupId, long startTime, long duration, boolean autoNotify, List<String> inviteList) {
        ...
        list.add(CubeCore.getInstance().getCubeId());
    ...
    @Override
    public TimePickerView initTimePicker(final Context context) {//Dialog 模式下，在底部弹出
        ...
        }).setCancelColor(context.getResources().getColor(R.color.C3)).setRangDate(selectedDate, endDate).setType(new boolean[] { true, true, true, true, true, false }).isDialog(true) //默认设置false ，内部实现将DecorView 作为它的父控件。
          .build();
