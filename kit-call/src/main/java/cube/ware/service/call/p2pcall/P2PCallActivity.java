 ...
 */
@Route(path = CubeConstants.Router.P2PCallActivity)
public class P2PCallActivity extends BaseActivity<P2PCallPresenter> implements CallStateListener, P2PCallContract.View {
    ...
    private MemberAdapter joinedMemberAdapter;//已加入成员适配器
    ...
    private void showAudioCallViewStub() {
        ...
        avatarList.add(CubeCore.getInstance().getAvatarUrl() + CubeCore.getInstance().getCubeId());
        avatarList.add(CubeCore.getInstance().getAvatarUrl() + this.mCallId);
        MemberAdapter adapter = new MemberAdapter(avatarList, this);
    ...
    private void showVideoCallViewStub() {
        ...
        memberList.add(CubeCore.getInstance().getAvatarUrl() + CubeCore.getInstance().getCubeId());
        memberList.add(CubeCore.getInstance().getAvatarUrl() + this.mCallId);
        joinedMemberAdapter = new MemberAdapter(memberList, this);
        LinearLayoutManager ms = new LinearLayoutManager(this);
        ms.setOrientation(LinearLayoutManager.HORIZONTAL);// 设置 recyclerview 布局方式为横向布局
        members_recycleview.setLayoutManager(ms);
